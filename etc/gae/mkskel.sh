...
WORK=~/Code # The working directory where narwhal, jack, jack-servlet and nitro reside.

SKEL=$WORK/skel # the skeleton project dir to be created
EPRJ=$WORK/nitro/etc/gae/skel # the origin gae project (created in Eclipse)
SAPP=$WORK/nitro/examples/simple # a simple app to use as starting point

WINF=$SKEL/war/WEB-INF

# The origin gae project is included for your convenience. It was created
# with the following procedure:
#
# 1. create a new Eclipse GAE Web application project with name 'JackSkell' and 
#    package 'org.jackjs' 
# 2. Set the servlet mapping path to '/*'.
# 3. Add to the following init param:
#		<init-param>
