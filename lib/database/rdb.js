...
var Database = exports.Database = function(config) {
    var uri = "jdbc:" + config.system + "://" + config.host + "/" + config.name + 
              "?user=" + config.user + "&password=" + config.password;
    this.conn = JDriverManager.getConnection(uri);
};

/**
 * Register a specific database adapter implementation.
 */
Database.setAdapter = function(name) {
    name = name || "com.mysql.jdbc.Driver";
    JClass.forName(name).newInstance();
    return this;
}
