...
var JSONResponse = require("nitro/utils/jsonp").JSONResponse;

/**
 * Render middleware.
 */
exports.Render = function(app, templateFilter) {

    var cache = {};
    
    templateFilter = templateFilter || require("nitro/utils/layout").filter;
    
    var loadTemplate = function(path) {
        var template = cache[path];
        if (!template) {
            if (file.exists(path)) {
                template = new Template(templateFilter(file.read(path).toString()));
