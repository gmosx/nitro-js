...
exports.Errors = function(app, options) {
    ...
    var templateNotFoundPath = options.errorTemplate || "/notfound.html",
        templateErrorPath = options.errorTemplate || "/error.html";
    ...
    return function(env) {
        ...
        if (status >= 400) {
            ...
            try {
                if (status < 500) {
                    try {
                        response.body = [env.render(templateNotFoundPath, response.data)];
                    } catch (e) {
                        response.body = [env.render(templateErrorPath, response.data)];
                    }
                } else {
                    response.body = [env.render(templateErrorPath, response.data)];
                }
